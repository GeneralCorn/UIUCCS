...
void ImagePPM::RemoveVert(const int* vertseam) {
  auto** vert_pix = new Pixel*[height_];
  for (int row = 0; row < height_; ++row) {
    vert_pix[row] = new Pixel[width_ - 1];
  }
  for (int row = 0; row < height_; ++row) {
    for (int col = 0; col < width_ - 1; ++col) {
      if (col >= vertseam[row]) {
        vert_pix[row][col] = GetPixel(row, col + 1);
        continue;
      }
      vert_pix[row][col] = GetPixel(row, col);
    }
  }
  for (int row = 0; row < height_; ++row) {
    delete[] pixels_[row];
...
void ImagePPM::RemoveHoriz(const int* horizseam) {
  auto** horiz_pix = new Pixel*[height_ - 1];
  for (int row = 0; row < height_ - 1; ++row) {
    horiz_pix[row] = new Pixel[width_];
  }
  for (int col = 0; col < width_; ++col) {
    for (int row = 0; row < height_ - 1; ++row) {
      if (row >= horizseam[col]) {
        horiz_pix[row][col] = GetPixel(row + 1, col);
        continue;
      }
      horiz_pix[row][col] = GetPixel(row, col);
    }
  }
  for (int row = 0; row < height_; ++row) {
    delete[] pixels_[row];
