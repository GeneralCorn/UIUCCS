...
int main(/*int argc, char* argv[]*/) {
  ImagePPM newimage("./images/image1.ppm");
  SeamCarver carve(newimage);
  std::ofstream test{"file.ppm"};

  for (int i = 0; i < carve.GetImage().GetWidth(); ++i) {
    std::cout << carve.GetHorizontalSeam()[i];
  }
  test << carve.GetImage();
}
